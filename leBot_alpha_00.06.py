...
PROFIT_THR_TO_OPEN_POSITIONS =  +0.000200     # open position threshold (values for testing the bot)
PROFIT_THR_TO_CLOSE_POSITIONS = -0.000100     # close positions threshold. Close all the positions openend by the arb thread.
...
def cross(exch_pair, coin_pair, coin_pairs_avail):
    ...
    if  exch_pair[0].rateLimit * 1.2 - (current_milli_time() - exch_pair[0].lastRestRequestTimestamp) > 0:
        time.sleep((exch_pair[0].rateLimit * 1.2 - (current_milli_time() - exch_pair[0].lastRestRequestTimestamp))/1000)
    ...
    if  exch_pair[1].rateLimit * 1.2 - (current_milli_time() - exch_pair[1].lastRestRequestTimestamp) > 0:
        time.sleep((exch_pair[1].rateLimit * 1.2 - (current_milli_time() - exch_pair[1].lastRestRequestTimestamp))/1000)
    ...
    if bid_1 and bid_2 and ask_1 and ask_2:
        ...
        if ((bid_1 - ask_2)/ask_2 - 2*(fee_1+fee_2)) >= PROFIT_THR_TO_OPEN_POSITIONS:
        ...
        elif ((bid_2 - ask_1)/ask_1 - 2*(fee_1+fee_2)) >= PROFIT_THR_TO_OPEN_POSITIONS:
...
def exploit_pair(exch_pair, coin_pair, coin_pairs_avail, reverse=False, ):
    ...
    if len(g_storage.exploit_threads) < MAX_THREADS:
        g_storage.exploit_thread_name += 1
        if not reverse:
            ...
            thread = threading.Thread(target=exploit_thread,
                                      name='arb_' + str(g_storage.exploit_thread_name) + '_' + exch_pair[0].name + '_' + exch_pair[1].name + '_' + coin_pair,
            ...
            thread = threading.Thread(target=exploit_thread,
                                      name='arb_' + str(g_storage.exploit_thread_name) + '_' + exch_pair[1].name + '_' + exch_pair[0].name + '_' + coin_pair,
...
def exploit_thread(exch_0, exch_1, coin_pair, coin_pairs_avail):
    ...
    while True:
        ...
        if  exch_0.rateLimit * 1.2 - (current_milli_time() - exch_0.lastRestRequestTimestamp) > 0:
            time.sleep((exch_0.rateLimit * 1.2 - (current_milli_time() - exch_0.lastRestRequestTimestamp))/1000)
        ...
        if  exch_1.rateLimit * 1.2 - (current_milli_time() - exch_1.lastRestRequestTimestamp) > 0:
            time.sleep((exch_1.rateLimit * 1.2 - (current_milli_time() - exch_1.lastRestRequestTimestamp))/1000)
        ...
        profit = (bid - ask)/ask - 2*(fee_0 + fee_1)
        ...
        if profit >= PROFIT_THR_TO_OPEN_POSITIONS:
            ...
            if base_coin_balance_0 >= trading_size_0 * (1+fee_0) and quote_coin_balance_1 >= (trading_size_1 * (1+fee_1) * ask):  # this is the ideal situation: money in both coins
            ...
            elif balances.get_coin_balance(exch_0.name, 'USD')['amount'] >= TRADING_SIZE and quote_coin_balance_1 >= 1.3 * (trading_size_1 * (1+fee_1) * ask):
                    logger.info('{} cash in USD in first exch and cash in quote coin in second exch'.format(thread_name))
                    # TODO: exch0: buy quote coin using USD, exch1: buy base coin (using quote)


            # option 2: base and USD
            elif base_coin_balance_0 >= trading_size_0 * (1+fee_0) and balances.get_coin_balance(exch_1.name, 'USD')['amount'] >= TRADING_SIZE:
                    logger.info('{} cash in base coin in first exch and cash in USD in second exch'.format(thread_name))
                    # TODO: exch0: sell base coin (using quote), exch1: buy base coin using USD
            
            # option 3: USD and USD
            elif balances.get_coin_balance(exch_0.name, 'USD')['amount'] >= TRADING_SIZE and balances.get_coin_balance(exch_1.name, 'USD')['amount'] >= TRADING_SIZE:
                    logger.info('{} cash in USD coin in first exch and cash in USD in second exch'.format(thread_name))
                    # TODO: buy quote using USD exch1: buy base using USD
            
            # option 4: USD and BTC
            elif balances.get_coin_balance(exch_0.name, 'USD')['amount'] >= TRADING_SIZE and balances.get_coin_balance(exch_1.name, 'BTC')['amount'] >= balances.get_coin_balance(exch_1.name, 'BTC')['trading_size']:
                    logger.info('{} cash in USD coin in first exch and cash in BTC in second exch'.format(thread_name))
                    # TODO: 
            
            # option 5: BTC and USD
            elif balances.get_coin_balance(exch_0.name, 'BTC')['amount'] >= balances.get_coin_balance(exch_0.name, 'BTC')['trading_size'] and balances.get_coin_balance(exch_1.name, 'USD')['amount'] >= TRADING_SIZE:
                    logger.info('{} cash in USD coin in first exch and cash in USD in second exch'.format(thread_name))
                    # TODO
            
            # option 6: BTC and BTC
            elif balances.get_coin_balance(exch_0.name, 'BTC')['amount'] >= balances.get_coin_balance(exch_0.name, 'BTC')['trading_size'] and balances.get_coin_balance(exch_1.name, 'BTC')['amount'] >= balances.get_coin_balance(exch_1.name, 'BTC')['trading_size']:
                    logger.info('{} cash in USD coin in first exch and cash in USD in second exch'.format(thread_name))            
                    # TODO
            
            else:
                logger.warning('{}: not enough cash for ordering selling-buying'.format(thread_name))
                # logger.warning('{} trying to RE-BALANCE...'.format(thread_name))

                # if not balancer([exch_0, exch_1], coin_pair):
                #     logger.error('{} REBALANCING was not possible'.format(thread_name))
                # else:
                #     logger.info('{} REBALANCING successful'.format(thread_name))
                
        else:
            logger.info('{}: trading not possible in \t{} and \t{} for \t{}, profit: \t{}'.format(thread_name, exch_0.name, exch_1.name, coin_pair, profit))
...
def main():
    ...
    while not g_storage.prices_updated:
        ...
        time.sleep(1)
    fake_balances(exchanges, 1000)  # TODO: to remove
