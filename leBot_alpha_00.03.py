...
def init_balances(exchanges):
    ...
    for exchange in exchanges:               # coin    balance              change USDT    trading size
        ...
        balances.set_balance(exchange.name, 'ETH',    0.633  * FACTOR,          158.0,          0.127  * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'LTC',    2.26   * FACTOR,           42.43,         0.5    * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'EOS',   40.0    * FACTOR,            2.50,        10.0    * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'XMR',    1.888  * FACTOR,           52.97,         0.5    * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'XRP',  523.0    * FACTOR,            0.191,       10.0    * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'ZEC',    2.78   * FACTOR,           36.25,         0.6    * SIZE_FACTOR)

        balances.set_balance(exchange.name, 'DASH',    1.38  * FACTOR,           73.09,         .3    * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'NEO',    13.8   * FACTOR,            7.27,         2.75   * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'PIVX',  333.34  * FACTOR,            0.3,          6.7    * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'NANO',  182.0   * FACTOR,            0.55,        36.36   * SIZE_FACTOR)
        balances.set_balance(exchange.name, 'ADA',  3030.34  * FACTOR,            0.033,      606.06   * SIZE_FACTOR)
        ...
        with open(filename, 'w') as csv_file:
            if profit >= PROFIT_THR_TO_OPEN_POSITIONS:
                ...
                if (base_coin_balance_0 >= trading_size_0 * (1+fee_0)) and (quote_coin_balance_1 >= (trading_size_1 * (1+fee_1) * ask)):
                    ...
                    logger.warning('Thread {} REBALANCING NEEDED'.format(thread_number))

                    # logger.info('Thread {}: calling exchange_balancer(exch_0, base_coin)'.format(thread_number))
                    # exchange_balancer(exch_0, base_coin)
                    # logger.info('Thread {}: calling exchange_balancer(exch_1, quote_coin)'.format(thread_number))
                    # exchange_balancer(exch_1, quote_coin)
...
def exchange_balancer(exchange, coin_dest):
    ...
    pairs_avail = [pair for pair in exchange.markets.keys()]

    for coin, coin_name in zip(balances.exchanges[exchange.name].values(), balances.exchanges[exchange.name].keys()):
        if not coin['in_use'] and str(coin_name + '/' + coin_dest) in pairs_avail and coin['amount']*coin['change'] > TRADING_SIZE:
            symbol = str(coin_name + '/' + coin_dest)
            amount = coin['amount'] * 1/2
            selling_price = get_selling_price(exchange, symbol, amount)
            if selling_price:
                fee = max(exchange.fees['trading']['maker'], exchange.fees['trading']['taker'])
                logger.info('exchange_balancer({}, {}): launching a selling order'.format(exchange.name, coin_dest))
                selling_order_demo(exchange, symbol, selling_price, amount, fee)
                return 0

        elif not coin['in_use'] and str(coin_dest + '/' + coin_name) in pairs_avail and coin['amount']*coin['change'] > TRADING_SIZE:
            symbol = str(coin_dest + '/' + coin_name)
            amount = coin['amount'] * 1/2
            buying_price = get_buying_price(exchange, symbol, amount)
            if buying_price:
                fee = max(exchange.fees['trading']['maker'], exchange.fees['trading']['taker'])
                logger.info('exchange_balancer({}, {}): launching a buying order'.format(exchange.name, coin_dest))
                buying_order_demo(exchange, symbol, buying_price, amount/buying_price, fee)
                return 0

    logger.error('exchange_balancer({}, {}): balancing on exchange was not possible'.format(exchange.name, coin_dest))
    return -1
...
'''
def launch_console():
    thread = threading.Thread(target=BotConsole().cmdloop())
    thread.start()
    thread.join()
    return

class BotConsole(cmd.Cmd):
    intro = 'Welcome to the leBot shell.   Type help or ? to list commands.\n'
    prompt = '(leBot): '
    file = None
    def do_show_config(self, arg):
        ...
        print('CROSSING_MARGIN: ', CROSSING_MARGIN)
...
def main():
    ...
    # testing balancer
    # exchange_balancer(exchanges[2], 'USDT')
